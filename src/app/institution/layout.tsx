...
import { LayoutDashboard, FileText, ClipboardList, Wrench, Bell, LogOut } from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col selection:bg-teal-500/20">
      ...
      <aside className="fixed top-0 left-0 bottom-0 w-60 bg-card border-r border-border/80 z-40 hidden md:flex flex-col">
        <div className="p-6 border-b border-border/40">
          <Link href="/institution/dashboard" className="flex items-center gap-3">
            <div className="w-10 h-10 bg-teal-600 rounded-xl flex items-center justify-center shadow-md shadow-teal-600/20">
              <span className="text-white font-bold">CC</span>
            </div>
            <div>
              <h1 className="font-bold text-foreground text-sm">Co-opConnect</h1>
              <p className="text-[11px] text-teal-400 font-medium">Institution</p>
            </div>
          </Link>
        </div>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-teal-500/15 text-teal-400 font-semibold border-l-2 border-teal-500"
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-card/90 backdrop-blur-md border-b border-border/80 z-40 flex items-center justify-between px-4">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 bg-teal-600 rounded-lg flex items-center justify-center text-white font-bold text-xs">CC</div>
          <span className="font-bold text-sm text-foreground">Institution</span>
        </div>
        <div className="flex items-center gap-2">
          <button 
